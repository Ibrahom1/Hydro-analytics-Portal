...
COMBINED_CSV_PATTERN = os.path.abspath(os.path.join(APP_DIR, '..', 'data', 'historical_river_data', 'combined_river_data_*.csv'))
...
# ---- GitHub Remote Sync Module ----
GITHUB_RAW_BASE = 'https://raw.githubusercontent.com/Ibrahom1/hydrosituation/main'
SYNC_INTERVAL_SECONDS = 300  # 5 minutes
_last_sync_status = {'success': None, 'message': '', 'timestamp': None}

def sync_from_github():
    """Download latest hydro_history.db and latest.json from GitHub."""
    global _last_sync_status
    results = []
    
    files_to_sync = [
        ('hydro_history.db', DB_PATH),
        ('latest.json', os.path.abspath(os.path.join(APP_DIR, '..', 'data', 'latest.json'))),
    ]
    
    for filename, local_path in files_to_sync:
        url = f'{GITHUB_RAW_BASE}/{filename}'
        try:
            resp = requests.get(url, timeout=60, headers={'User-Agent': 'HydroAnalytics-Sync/1.0'})
            if resp.ok:
                ...
                results.append(f'{filename}: OK ({len(resp.content):,} bytes)')
                logging.info(f'GitHub sync: {filename} updated ({len(resp.content):,} bytes)')
            else:
                results.append(f'{filename}: HTTP {resp.status_code}')
                logging.warning(f'GitHub sync: {filename} failed with HTTP {resp.status_code}')
        except Exception as e:
            results.append(f'{filename}: ERROR {e}')
            logging.error(f'GitHub sync: {filename} failed: {e}')
    
    # Reload latest.json into cache after sync
    latest_json_path = os.path.abspath(os.path.join(APP_DIR, '..', 'data', 'latest.json'))
    if os.path.exists(latest_json_path):
        try:
            ...
            if ok:
                results.append(f'Cache refreshed from latest.json')
        except Exception as e:
            results.append(f'Cache refresh failed: {e}')
    
    _last_sync_status = {
        'success': all('OK' in r for r in results[:2]),
...
def _background_sync_loop():
    """Background thread that syncs from GitHub every SYNC_INTERVAL_SECONDS."""
    import time
    while True:
...
def start_background_sync():
    """Start the background sync thread (called once on startup)."""
    sync_thread = Thread(target=_background_sync_loop, daemon=True)
    sync_thread.start()
    logging.info(f'Background GitHub sync started (every {SYNC_INTERVAL_SECONDS}s)')
...
def sync_remote():
    """Fetch latest hydro_history.db and latest.json from GitHub"""
...
if __name__ == '__main__':
    ...
    # Initial sync from GitHub and start background sync
    logging.info('Performing initial sync from GitHub...')
    sync_from_github()
    start_background_sync()

    # Load latest.json into telemetry cache on startup
    latest_json_path = os.path.abspath(os.path.join(APP_DIR, '..', 'data', 'latest.json'))
    if os.path.exists(latest_json_path):
        try:
            with open(latest_json_path, 'r', encoding='utf-8') as f:
                payload = json.load(f)
            apply_remote_dataset(payload)
            logging.info('Loaded telemetry cache from latest.json')
        except Exception as e:
            logging.warning(f'Could not load latest.json on startup: {e}')
